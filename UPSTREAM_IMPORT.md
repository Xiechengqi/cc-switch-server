...
| 日期 | 上游提交/范围 | 分类 | 处理 | 说明 |
| --- | --- | --- | --- | --- |
| 2026-07-20 | 9router Grok CLI 0.2.99 identity and non-numeric quota state；sub2api/TokenRouter weekly+monthly billing merge；cockpit-tools subscription/task usage and credit-bag parsing | Must Review / Grok OAuth quota + subscription metadata | 已选择性移植并强化 | Grok quota refresh 统一使用 `grok-shell/0.2.99` 指纹，并并发探测 user、weekly billing、monthly billing 和 task usage，缺订阅到期证据时 best-effort 查询 `/rest/subscriptions`。新增 weekly/product/monthly/task/credit-bag 规范化、per-probe status/warning/stale 合并与 402 spending-limit 状态；API/UI 区分 numeric、non-numeric、partial、unavailable，并禁止将 token expiry、`currentPeriod.end` 或 `billingPeriodEnd` 当作订阅到期。只接受 active subscription 内明确的 expiry 字段，否则允许按账号手动设置下一次付款到期时间；手动值在 OAuth 刷新后保留并同步到 provider/Share 元数据，但不影响凭据有效性或代理调度。不迁移 grok.com web-cookie 逆向、Skill/MCP/Tauri/Desktop 能力。 |
| 2026-07-20 | TokenRouter `backend/internal/service/openai_privacy_service.go` ChatGPT account candidate filtering；本地生产 `/wham/usage`、`/accounts/check` 诊断证据 | Must Review / Codex OAuth quota + subscription metadata | 已选择性移植并强化 | 吸收 expired/deactivated/disabled/inactive/suspended account candidate 过滤；server 进一步将 `/wham/usage.plan_type` 设为展示套餐权威，只允许已验证 workspace 的 `/accounts/check` 与 `/subscriptions` 提供到期日，禁止可信 workspace 精确匹配失败后跨账号回退。套餐冲突、无可信 workspace 的到期日、以及付费 usage 仍可用时的历史过期日均丢弃，并持久化不含凭据的裁决证据；刷新后的空到期日会同步清除 Auth 与 Share descriptor 的错误 expired 状态。不迁移 TokenRouter 的代理/TLS/管理面实现。 |
