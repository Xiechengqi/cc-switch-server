...
codebuddy_scope_prefix() {
  printf '%s' "${1^^}" | tr '-' '_'
}

codebuddy_gate_status() {
  local prefix
  prefix="$(codebuddy_scope_prefix "$1")"
  gate_status \
    SERVER_URL CC_SWITCH_SERVER_TOKEN CC_SWITCH_SHARE_URL ROUTER_API_TOKEN \
    "CODEBUDDY_${prefix}_TEST_ACCOUNT" \
    "CC_SWITCH_CODEBUDDY_${prefix}_CLAUDE_PROVIDER_ID" \
    "CC_SWITCH_CODEBUDDY_${prefix}_CODEX_PROVIDER_ID" \
    "CC_SWITCH_CODEBUDDY_${prefix}_GEMINI_PROVIDER_ID" \
    "CC_SWITCH_CODEBUDDY_${prefix}_SHARE_ID" \
    "CC_SWITCH_CODEBUDDY_${prefix}_MODEL" \
    "CODEBUDDY_${prefix}_REAL_RECEIPT_FILE"
}

check_codebuddy_receipt_scope() {
  local site="$1"
  local prefix
  prefix="$(codebuddy_scope_prefix "$site")"
  check_external_group \
    "AB7 CodeBuddy ${site} receipt" \
    SERVER_URL CC_SWITCH_SERVER_TOKEN CC_SWITCH_SHARE_URL ROUTER_API_TOKEN \
    "CODEBUDDY_${prefix}_TEST_ACCOUNT" \
    "CC_SWITCH_CODEBUDDY_${prefix}_CLAUDE_PROVIDER_ID" \
    "CC_SWITCH_CODEBUDDY_${prefix}_CODEX_PROVIDER_ID" \
...
if should_run "AB7"; then
  ...
  echo "== AB7 CodeBuddy immutable-site private receipt gates =="
  check_codebuddy_receipt_scope intl
  check_codebuddy_receipt_scope cn
  echo "[INFO] Run node scripts/smoke/codebuddy-real-receipt.mjs once per site; input readiness is not live acceptance."
...
if [[ -n "$EVIDENCE_FILE" ]]; then
  ...
  QODER_GLOBAL_OAUTH_GATE_STATUS="$(gate_status QODER_GLOBAL_OAUTH_TEST_ACCOUNT SERVER_URL CC_SWITCH_SERVER_TOKEN CC_SWITCH_SHARE_URL ROUTER_API_TOKEN CC_SWITCH_QODER_GLOBAL_OAUTH_CLAUDE_PROVIDER_ID CC_SWITCH_QODER_GLOBAL_OAUTH_CODEX_PROVIDER_ID CC_SWITCH_QODER_GLOBAL_OAUTH_GEMINI_PROVIDER_ID CC_SWITCH_QODER_GLOBAL_OAUTH_SHARE_ID)" \
  QODER_GLOBAL_PAT_GATE_STATUS="$(gate_status QODER_GLOBAL_PAT_TEST_ACCOUNT SERVER_URL CC_SWITCH_SERVER_TOKEN CC_SWITCH_SHARE_URL ROUTER_API_TOKEN CC_SWITCH_QODER_GLOBAL_PAT_CLAUDE_PROVIDER_ID CC_SWITCH_QODER_GLOBAL_PAT_CODEX_PROVIDER_ID CC_SWITCH_QODER_GLOBAL_PAT_GEMINI_PROVIDER_ID CC_SWITCH_QODER_GLOBAL_PAT_SHARE_ID)" \
  QODER_CN_OAUTH_GATE_STATUS="$(gate_status QODER_CN_OAUTH_TEST_ACCOUNT SERVER_URL CC_SWITCH_SERVER_TOKEN CC_SWITCH_SHARE_URL ROUTER_API_TOKEN CC_SWITCH_QODER_CN_OAUTH_CLAUDE_PROVIDER_ID CC_SWITCH_QODER_CN_OAUTH_CODEX_PROVIDER_ID CC_SWITCH_QODER_CN_OAUTH_GEMINI_PROVIDER_ID CC_SWITCH_QODER_CN_OAUTH_SHARE_ID)" \
  CODEBUDDY_INTL_GATE_STATUS="$(codebuddy_gate_status intl)" \
  CODEBUDDY_CN_GATE_STATUS="$(codebuddy_gate_status cn)" \
