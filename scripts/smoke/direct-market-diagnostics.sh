...
stream_probe_summary() {
  ...
  echo "== ${label} =="
  echo "url=${url}"
  if node scripts/smoke/stream-probe.mjs "${args[@]}" >"$summary_file"; then
    ok=1
  else
    ok=0
  fi
...
if [[ "${#auth_header[@]}" -eq 0 ]]; then
  ...
  fetch_summary "Usage requests" "$SERVER_URL/web-api/usage/requests?limit=20" "${auth_header[@]}"
  USAGE_REQUESTS_STATUS="$FETCH_STATUS"
...
if [[ -n "$EVIDENCE_FILE" ]]; then
  ...
  USAGE_REQUESTS_STATUS="$USAGE_REQUESTS_STATUS" \
