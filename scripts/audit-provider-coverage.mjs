...
const jsonPath = path.join(repoRoot, "docs/provider-coverage.json");
...
if (checkMode) {
  ...
  if (actualJson !== json || actualMd !== markdown) {
    throw new Error("provider coverage docs are out of date; run scripts/audit-provider-coverage.mjs");
  }
  console.log("provider coverage docs are up to date");
} else {
  const changed =
    writeIfChanged(jsonPath, json) | writeIfChanged(mdPath, markdown);
  console.log(changed ? "provider coverage docs updated" : "provider coverage docs unchanged");
}
