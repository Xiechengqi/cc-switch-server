...
function serverEvidenceNotes(qoderVerification) {
  return [
    ...
    "",
    "- Typed Provider ownership is derived from immutable `profileId`; fixed Profiles ignore conflicting name, URL, category, and raw `meta.providerType` hints. Only S1/`legacy_compat` records retain endpoint/name heuristics. Native official Claude OAuth, Anthropic API Key, OpenAI OAuth, and OpenAI API Key Profiles default to `modelMapping.mode=passthrough` and may be switched explicitly to `single` with the Registry-owned `defaultUpstreamModel`. Native official Google Profiles remain locked to passthrough and retain the requested text model.",
    ...
    "- Provider load performs only an in-memory compatibility normalization and never rewrites `providers.json`. An allowed explicit mode and actual model are preserved; missing mappings use the Profile default policy, and a missing single-model mapping uses the Profile default upstream model. Legacy model values are inferred from app configuration when possible, and legacy Grok providers without an explicit mapping default to `grok-4.6`. S1-to-S2 cutover is an explicit offline CLI action; unresolvable historical records block cutover rather than being guessed.",
    ...
    "",
    "### OpenAI Chat response compatibility",
    "",
    "- Every successful `/v1/chat/completions` response uses a positive integer Unix-seconds `created`. Streaming role, text, reasoning, tool, finish, and usage-only chunks share one frozen value: the first valid upstream `created`/`created_at` observed before downstream commit wins, otherwise Server captures one fallback clock value. Missing, null, non-integer, non-positive, or later drifting values are normalized at the common Chat output boundary for Responses/Anthropic/Gemini synthesis, native Chat passthrough, managed OAuth drivers, API-key Coding Plans, Custom HTTP, and dedicated canonical emitters. Non-stream Chat success envelopes receive the same validation; error envelopes and `[DONE]` are not rewritten as successes.",
    "- The compatibility layer is incremental and event-bounded, preserves untouched SSE event bytes and control metadata, and exposes `cc_switch_openai_chat_compat_total{action,path}` with production `action=preserved|created_injected|created_rewritten|invalid_chunk` and `path=synthesized|passthrough`. Strict schema fixtures cover arbitrary byte fragmentation, LF/CRLF/lone-CR framing, multi-line data, role/text/reasoning/tool/finish/usage chunks, invalid timestamps, and stream-wide stability. Real Grok OAuth/Grok CLI acceptance remains `live_pending` until private inputs are available.",
    "",
    "### Provider control plane and storage",
    "",
    "- Rust `ProfileSpec` is the product identity authority, `DriverSpec` owns protocol operations, and each committed Provider compiles one canonical `RuntimePlan` shared by forwarding, manual test, and model discovery. Custom Profiles derive compatibility type deterministically from their explicit upstream protocol and authentication scheme; Anthropic Messages with Bearer authentication is classified as `claude_auth`. Named Custom recipes remain convenience configuration, not separate Provider families.",
    ...
    "",
    "### Provider-owned Web Session rail",
    "",
    "- The Registry adds four hidden typed Profiles: Grok Web Session and Perplexity Web Session on both Claude and Codex. They compile to two high-risk Experimental Drivers and one Provider-owned Cookie slot; Account binding, API-key slots, extra headers, credential pooling, rotation, and every cross-account/cross-Provider/cross-credential fallback are forbidden. The Profiles remain hidden until separate live acceptance is complete.",
    "- Each Driver fixes the reviewed POST origin and path, Origin, Referer, browser identity, allowed Cookie names and canonical Cookie order. A dedicated HTTP client disables redirects and cookie storage; cross-origin movement is rejected, client Authorization and arbitrary headers are never forwarded, and upstream `Set-Cookie`/`Location` are neither persisted nor returned. Requests and responses are bounded before strict parsing.",
    "- Claude Messages and local `count_tokens`, plus Codex Responses and Chat Completions, support text-only non-stream and synthesized stream lifecycles. Tools, function history, images, audio, files, and unsupported routes fail locally before any upstream request. Grok NDJSON and Perplexity SSE are consumed under first-frame, idle, total, frame, and body bounds and require their reviewed terminal; malformed, truncated, post-terminal, or cancelled lifecycles never retry or fall through another Driver.",
    "- Runtime authentication state is scoped by App/Profile, Provider key and revision, runtime fingerprint, credential generation, fixed origin, and reviewed profile identity. A 401/403 invalidates only that exact generation; another request performs zero network work until an explicit Cookie re-import advances the generation. Provider commits, reloads, runtime-default changes, deletion, and test-store replacement retain only still-valid scopes through `state.rs` domain methods.",
    "- Discovery is a reviewed static text catalog with `source=reviewed_web_session_catalog`, `stale=false`, `fixture_verified`, `live_pending`, and `entitlement=not_asserted`; it never interprets the catalog as live subscription entitlement. `assets/contract/web-session-source-baseline.json` pins OmniRoute `b342c1a361f2` and 9router `90b52e06ffd6` evidence hashes, while `scripts/audit/audit-web-session-registry.mjs` gates the generated manifest and source drift.",
    "",
    ...
    "- Local client, state, API, query, and rendering contracts cover methods/headers, body bounds, redirect refusal, error privacy, partial/stale merge, Bundle singleflight, post-commit warmup, deletion/rotation fences, in-flight old-generation discard, session authorization, response redaction, revision scope, and zero-value display. The generated coding-plan manifest independently checks these source tokens and fixture names. `cargo test ollama_cloud_live_account_usage_from_env -- --ignored` is an optional real-account smoke requiring `OLLAMA_API_KEY`; a missing environment input is not live evidence.",
    "",
    ...
    "- `oauth.gemini_code_assist` model discovery force-refreshes quota for the one `gemini_cli` Account explicitly bound to the Provider and derives a deduplicated Gemini-only catalog from `retrieveUserQuota.buckets[].modelId`. One eligible 401 may refresh and replay that same Account. A transient refresh failure may expose only a same-`authIdentityGeneration` cached catalog with `stale=true`; an absent or cross-generation cache fails closed, and no static or other-account catalog is merged.",
    ...
    "- Antigravity and Agy additionally expose a distinct `antigravity_code_plan` projection without replacing the generic Gemini projection. Its `project_bootstrap`, `privacy`, `tier_entitlement`, authenticated model-catalog, Gemini/Claude/GPT quota-family, and model-capacity dimensions are current-generation and expiring. Project/tier evidence comes from `loadCodeAssist`; quota refresh still records explicit `retrieveUserQuota` buckets, while Provider discovery uses the exact bound account's `fetchAvailableModels` response for model quota/reset, thinking/image/token/MIME capability, deprecated aliases, and family evidence. Catalog cache scope is Provider type + Account + `authIdentityGeneration`; Agy and Antigravity never share it. Success-empty is authoritative, only network/408/429/5xx may use same-identity stale data, and 401/403 or binding drift fails closed. A bounded `fetchUserInfo` probe observes privacy read-only: an empty `userSettings` object is positive evidence, any `telemetryEnabled` field is negative evidence, and missing fields or probe failures are unknown. The Server never invokes `setUserSettings`, invents a weekly quota summary, or stores project ids in capability evidence.",
    ...
    "- Model discovery calls the validated Account endpoint with the short-lived token and the managed Copilot editor/plugin identity. Catalog caches include Account id, `authIdentityGeneration`, Account token generation, short-lived Copilot token generation, GitHub domain, and validated API origin. The upstream catalog is authoritative even when successfully empty; policy-disabled, picker-disabled, non-chat, and non-routable models are filtered while safe capability/limit metadata is retained. Only network failures, 408, 429, and 5xx may read a same-scope stale cache. github.com and GHES both fail closed on 401/403, malformed success payloads, binding/generation drift, or missing same-scope cache; neither uses the public static compatibility catalog at runtime.",
    ...
    "",
    "- DeepSeek password login remains excluded. Import accepts one raw bearer access token of at most 16 KiB plus non-secret account metadata; it rejects an embedded `Bearer ` prefix, whitespace/control characters, refresh/ID tokens, API keys, scopes, extra headers, cookies, passwords, session credentials, and equivalent nested profile/raw rails. Add/list/remove/default operations use the real encrypted `AccountStore` rather than compatibility success stubs.",
    "- The capability is `managerKind=import_only`, `credentialOwnership=managed_account`, `refreshCapability=unavailable`, and `quotaCapability=cached_only`. Imported quota snapshots may be displayed, but there is no browser login, password exchange, native token refresh, Cookie fallback, or live quota refresh claim. `deepseek_account` is distinct from Provider-owned `deepseek_api`; neither rail can materialize or recover the other.",
    "- Claude Messages uses a native DeepSeek Web bridge with production origin fixed to `https://chat.deepseek.com`. Session creation, PoW challenge/solution, and completion requests use one explicit Provider/Account binding. The bounded 30-minute single-flight session cache includes App, Provider revision/runtime, Account id plus auth/token generations, Share, signed user, client session, and reviewed model. Only a reused session rejected with 400/404/409 may be discarded and rebuilt once on that same scope before downstream commit; 401/403/429/5xx, a new-session failure, generation drift, or any post-commit failure is terminal.",
    "- The bridge preserves multi-turn text, thinking, search citations, and nonce-bound declared tool call/result continuity. PoW validation bounds algorithm, target, expiry, difficulty, challenge size, arithmetic, and work. Non-stream and SSE parsing recognize reviewed DeepSeek fragment/status/search shapes, require one strict terminal, reject data after terminal and malformed/truncated EOF, and never replay after downstream business output. Images remain unsupported; Codex and Gemini DeepSeek Web surfaces remain unsupported rather than falling through generic HTTP.",
    "- Provider test dry-run verifies the fixed origin, bearer-only materialization, reviewed model, explicit Account and generation without network traffic. Network mode executes the same native session -> PoW -> completion -> strict-terminal chain with an independent deadline and 4 MiB response cap. Model discovery requires the same bound Account/generation and returns only the reviewed text/tool/thinking/search catalog with explicit `live_pending` entitlement; it does not merge static, API-key, or other-account models.",
    "- Local account-import, session-scope/recovery, PoW, request/stream/tool/search/terminal, native provider-test, and reviewed-discovery contracts are `fixture_verified`. Real bearer token, non-stream/stream, thinking/search/tool, session expiry/rebuild, auth failure, model drift, and revocation receipts remain `live_pending`; no live DeepSeek Web success is claimed.",
    "",
    ...
    "",
    "Server-native Kiro protocol implementation reviewed through 2026-09-18 against OmniRoute `918fba5e392ce8b137976349f035597196edc440`, 9router `15223724c3e1ad898e84ef6e0cc1686cbafc8290`, the committed prompt-cache deltas in `kiro.rs@22d2c2d0695ba350890072c19990f54782827ae5`, and the namespaced-tool/profileArn/remote-compaction deltas through `kiro.rs@be0c04219d9d1b93b7fe5c3d7b9e7c9cf0d05863`. Local fixture verification does not imply live-account acceptance:",
    "",
    ...
    "- Tool and image hardening: top-level tool input schemas are forced to objects and unsupported combinators are stripped with object-field recovery. Tool input is withheld until `stop=true` and complete JSON validation, including across multiple frames; pending tool JSON is capped at 16 MiB, and invalid, incomplete, or oversized JSON returns a stable non-retryable 502 code. Claude Code built-in tool names and inputs are bridged in both directions. Bare names returned for namespaced tools are resolved only from current-request declarations after exact mapped/original matches; one child candidate is restored, an ordinary exact name wins, and multiple namespace candidates fail closed with `KIRO_EVENT_STREAM_INVALID`. History, Account, Share, session, and process state cannot supply candidates. Image inputs require string base64 `source.data` and use magic-byte MIME detection, bounded decoding/resizing/output budgets, newest-copy deduplication, and image extraction from tool results. `ksk_` values are masked before errors enter logs.",
    "- Discovery and caches: bound-account `ListAvailableModels` results replace rather than union with configured/static models, including an authoritative successful empty list. Identity and credential validation happens before cache lookup. Cache identity includes app, Provider id/revision, runtime fingerprint, `account_id`, `auth_identity_generation`, `token_refresh_generation`, authoritative profile ARN (or explicit profileless API-key marker), and runtime region. Only a transient network/timeout/408/429/5xx failure after identity validation may use that exact scope's bounded last-known-good catalog; without one, discovery fails closed. Authentication/authorization failure, unresolved enterprise identity, absent credentials, malformed JSON, a success payload without `models`/`availableModels`, an oversized response, and any Provider/runtime/account/generation drift cannot use stale or static models. A profileless compatibility retry is allowed only after a profile-bearing 403 or a 400 body explicitly reporting `Improperly formed request`/`Invalid profileArn`; 401, 429, 5xx, transport/TLS/timeout, decode, and malformed success do not trigger it. Usage retries stay on the same `q.{region}` host; the unproven CodeWhisperer-host fallback is disabled. The first discovery 401 may force-refresh the same bound Account and replay once; a second 401 is terminal. No model catalog is merged across accounts. Prompt-cache keys additionally namespace Provider, account, route, session, model and request-global inference options. Local metering supports top-level auto plus explicit breakpoints, a unified four-breakpoint limit, 20-position lookback with tool-use/result grouping, 1h-before-5m ordering, entry-own TTL renewal, and a latest-user guardrail. Invalid declarations fail to no-cache with a bounded reason. Local covered/read estimates are mapped by deterministic integer ratios onto the authoritative total, while upstream cache usage including explicit zero always wins; `cache_usage_source` distinguishes these paths. `count_tokens` uses a local estimator, sends no inference request, and records no generation usage.",
    "- Prompt-cache durability: request handling mutates only a bounded in-memory table and dirty generation map. A capacity-2 background writer coalesces mutations, uses bounded retry backoff, and commits transactional upserts/deletes plus generation CAS to WAL/FULL SQLite; disk failure does not block or fail inference and is exposed as degraded durability. Graceful shutdown performs a bounded flush. The configured base path receives a `.sqlite` suffix; an old JSON snapshot is imported once while its source file remains available for rollback. Shared/remote cache is runtime-disabled, and reference Redis, session affinity, account routing, and cache-hit account selection are excluded.",
    "- Amazon Q isolation: the reviewed OmniRoute `amazon-q` registry path aliases its Kiro OAuth provider, refresh helper, executor, and static Kiro-style catalog, so it is not independent Amazon Q entitlement evidence. Server instead has a separate `amazon_q_oauth` Account type and `special.amazon_q` Driver. Kiro credentials, profiles, catalogs, quota, endpoint origin, and request origin are never accepted for Amazon Q, and Amazon Q credentials are never accepted for Kiro. The two products share only reviewed pure translation, image, and EventStream-decoder code.",
    ...
    "- Evidence: `assets/contract/kiro-wire-protocol.json` and `assets/contract/kiro-reference-delta.json` are consumed directly by Rust and static audits for endpoint, image, decoder, runtime-model, three-surface bridge, prompt-cache semantics, usage conservation, SQLite recovery, request-scoped tool restoration, narrow profile fallback, and zero-network compaction rejection. KI-01 through KI-03 and KI-N1/KI-N2 are `fixture_verified`; KI-N3 is fixture-verified only for the fail-closed gate. KI-04 remains `live_pending` until Builder ID/IdC/Social/API Key are each tested in `us-east-1` and `eu-central-1` for Claude/Codex non-stream/stream, cache modes/TTLs/bounds, upstream usage, refresh, throttling, and stable errors. KI-05 shared cache and KI-N3 remote compaction remain runtime-disabled pending explicit need and real evidence. Amazon Q has its own fixture and live gate below.",
    "",
    ...
    "",
    "Claude OAuth protocol evidence review through 2026-09-03, implemented independently in Server:",
    "",
    "- Proxy hot path: legacy-compatible and typed Claude OAuth Providers share one prepared-request contract: managed-account refresh, `?beta=true`, ordered model/body-driven betas, CLI/Stainless identity, fail-closed native/helper classification, global cache-control constraints, preserve-order JSON, and one final CCH over the cleaned body. The npm-latest 2.1.258 profile derives a three-hex billing suffix from the original first user text with the official salted JavaScript UTF-16 algorithm, keeps the billing block uncached, uses CCH seed `0x4D659218E32A3268`, recursively blanks model strings, and excludes `max_tokens`, `fallbacks`, and `fallback_credit_token`; prompt `1e2` and CCH `8d393` golden vectors verify the result. UA or billing alone cannot enter native minimal passthrough. Known tools retain canonical casing and response round-trip restoration. Optional custom/MCP aliases are session-scoped, default off, collision-safe, and cover declarations, forced choice, history, JSON and SSE while skipping server tools. Unknown betas are dropped without raw logging; obsolete fine-grained-streaming/computer-use betas are not synthesized. Advisor is exact-shape gated for Messages and count_tokens; advanced-tool-use requires audited tool-search/deferred shapes, while thinking display updates requires the exact `updates` value and excludes redaction. Server fallback is never synthesized: arrays require the client-explicit 2026-06-01 beta and `default` requires 2026-07-01; mismatches are stripped before CCH.",
    "- Retry and semantic hardening: verified Router Share ingress resolves only the Share-bound Claude Surface and immutable Provider/account binding. Messages may replay once only for connect-stage failure, once after same-account 401 refresh, or through bounded body compatibility stages. 429/529 never replay. Claude 429 scope is explicit: unified 5h/7d rejection writes the bound account CAS cooldown; Fable-only applies only when `7d_oi` is rejected while both shared windows are `allowed` or `allowed_warning`. Missing/ambiguous shared evidence and rejected unified evidence fail closed to the account scope; ordinary and unknown 429 write only current Share+model cooldown; Fast credits refusal writes none. Reset parsing supports relative seconds, epoch seconds/milliseconds, RFC3339 and HTTP-date with a bounded horizon. `count_tokens` stays on the same identity. Native JSON/SSE validation and downstream cancellation semantics remain unchanged.",
    "- Response-header quota projection: managed Claude OAuth Messages sample only the reviewed `5h`, `7d`, and Fable `7d_oi` utilization/reset/status headers before JSON, SSE, and 429 handling. Samples are range-checked, monotonic, reset/TTL-bounded, and fenced to the exact bound Account identity generation; token-only refresh preserves them while principal replacement clears them. Active usage tiers remain authoritative and response observations only fill missing tiers. Fable projection requires a fresh eligible Max plan or direct successful/Fable-only rejection evidence, publishes model-family/capacity-pool metadata to Account and Router Share quota, and expires without writing an exhausted-pool marker for ordinary utilization. Generation-aware UI/Share updates are immediate for tier/reset/100% transitions and throttled for ordinary percentage changes. This path never selects, rotates, or falls back to another account.",
    ...
    "- Wire profile and discovery: `assets/contract/claude-oauth-wire-profile.json` records a sanitized static and fake-credential loopback audit of the official npm latest/next Claude Code `2.1.258` native binary, Stainless `0.112.1`, Node `v26.3.0`, and Axios `1.15.2`; npm stable was observed at 2.1.236. It excludes credentials, identifiers, raw bodies and private build metadata. It versions endpoint identities, prompt fingerprint/CCH/beta rules and the Fable 5.1, Fable 5, Opus 5, Opus 4.8, Sonnet 5, Opus/Sonnet 4.6 and Haiku 4.5 catalog. Fable 5 and 5.1 share the existing Max 5x/20x entitlement and weekly capacity pool without fallback. Discovery performs zero OAuth/upstream requests and remains fixture-verified; real subscription-account verification is pending.",
    ...
    "- Model discovery is single-flight and scoped by App, Provider revision/runtime, exact Account, auth identity generation, and token refresh generation. A successful empty catalog is authoritative; only reviewed wire models become aliases, while an unknown-only or malformed success fails closed. Only network/408/429/5xx failure can use bounded stale data in the identical scope; no static entitlement fallback remains.",
    "- K3 effort normalizes to `low`/`high`/`max` with default `max`; thinking keeps all signed blocks. Replay is bounded and scoped by App, Provider revision/runtime, Account auth and token generations, Share, hashed signed user, session, and model family. CAS writes/deletes revalidate binding and both credential generations; 400/422 deletes only an applied replay, streaming commits at `message_stop` rather than waiting for EOF, and hit/miss/rejection/expiry metrics carry no tenant labels.",
    "- Thirty-seven offline Kimi contract tests pass across three App surfaces, discovery, canonical signature/placeholder/parallel-tool replay, same-account single-401 recovery, and Provider/Account/credential-generation drift. Real device login, refresh, catalog, stream/tools/images, quota, error recovery, and entitlement acceptance remain external gates. See `docs/provider/kimi-code.md`.",
    "",
    "### `qoder_cosy` (Qoder OAuth)",
    "",
    "Server-owned capability independently implemented from official Qoder CLI oracle evidence, with bounded compatibility cross-checked read-only against TokenRouter `5f94cbcf2d1f4e74badf449c192c1431dc4e5c8e` and cli2api `b67278960df9c160d45a7520ee3110b5ccb84126`:",
    "",
    ...
    "- COSY requests use the reviewed body encoding, encryption, and fixed site endpoints. Signing timestamps must be within five minutes of the local clock and request ids must be canonical UUID v4 nonces. Live model configuration is authoritative, including a successful empty catalog; malformed entries and non-boolean enable flags fail closed. Catalog, session, and quota caches include the exact Provider/runtime/Account/site/auth-and-token-generation scope.",
    ...
    "- CN catalog, quota, auth-status, and generation derive a non-empty canonical IPv4 from the Server's outbound route, with an explicit environment override for remote-DNS deployments; downstream forwarded headers are never trusted. Global keeps its audited machine-identity client-IP rule.",
    "- Response normalization accepts reviewed OpenAI delta/final-message/content-block aliases and Anthropic-style content/tool/message envelopes without weakening the unique-terminal-plus-EOF contract. Tool history repairs missing/duplicate call ids within one assistant batch, filters unknown/orphan pairs, restores parallel result order, and rejects ambiguous missing result ids instead of guessing or binding across batches.",
    "- OAuth and data-plane failures distinguish invalid grant, authentication, permission, rate limit, temporary, and protocol classes. Retry-After milliseconds/seconds/HTTP-date are capped at 24 hours; recursive bounded JSON redaction covers credentials and identity fields. Device Flow keeps one active Qoder flow per admin principal, a 64-entry global cap, and a 60-second completed-result idempotency window.",
    ...
    "",
    "Cursor OAuth/API key protocol evidence review through 2026-08-30, implemented independently in Server:",
    "",
    ...
    "- Endpoint discovery and exchange: when no rail-specific override is configured, both rails authenticate `POST https://api2.cursor.sh/aiserver.v1.ServerConfigService/GetServerConfig`, decode exactly one protobuf `AgentUrlConfig` field 27, require exactly one `agentUrl` and `agentnUrl`, and reject duplicate/wrong-wire/missing values rather than inheriting first/last-wins ambiguity. Both values must be pathless, default-port HTTPS origins on `api5.cursor.sh` or its subdomains. Inference uses `agentUrl + /agent.v1.AgentService/Run`. The one-hour cache scope includes App, Provider revision, credential generation, runtime fingerprint, rail, principal, and access-token digest. API-key exchange defaults to `/auth/exchange_user_api_key`; OAuth refresh uses that endpoint with the same bound refresh token as Bearer plus `{}`. Discovery and AgentService share one same-credential 401 recovery budget, and optional endpoint overrides remain fail-closed HTTPS URLs.",
    ...
    "- Model modes and catalog: Share resolution fixes the Cursor Provider before parsing aliases, so model text never selects or escapes a Provider/Share. `cursor`, Agent/Plan/Ask/Composer aliases and `cursor:`, `cursor-agent:`, `cursor-plan:`, `cursor-ask:` prefixes resolve wire model/mode. A trailing `-fast` normally becomes a protobuf parameter; only a complete ID in a fresh API-key `/v1/models` catalog from the exact App, Provider revision/runtime, credential generation, and key-digest scope is sent verbatim. OAuth/static, stale, expired, or cross-scope catalogs cannot authorize this exception. Explicit selectors override only the current Cursor Provider's single-model default, while ordinary unprefixed requests retain RuntimePlan policy. API-key discovery materializes only the exact S2 Provider credential into zeroizing memory and uses the same exact scope for token, cooldown, and model caches. A successful empty catalog is authoritative; only retryable transport/429/5xx failure may use that scope's last-known-good for presentation, bounded to one hour. Gemini `/v1beta/models` uses this same exact-scope path, and non-conflicting bare IDs plus deterministic namespaced variants are collision-deduplicated.",
    ...
    "- Evidence: `cargo test cursor --lib` passes 289 keyword-filtered tests (including unrelated pagination-cursor tests). Relevant contracts cover endpoint discovery/trust and duplicate rejection, exact-scope sessions/indexes/catalogs, fresh verbatim model IDs, bounded stale, initial and resumed absolute deadlines, builtin rejection, credentials, all four response emitters, and signed Gemini Share catalog isolation. Driver forward/test conformance is `fixture_verified`; maturity remains Experimental and live acceptance is `live_pending`. Do not mark OAuth/API-key live until separate real accounts exercise discovery, streaming, tool call/result continuation, images, builtin/declared tools, and rate-limit/cooldown behavior.",
    "",
    ...
    "- Quality observation: successful Responses JSON, SSE, and native WebSocket paths feed a bounded read-only shape observer. `cc_switch_grok_quality_observations_total` exports only fixed outcome/transport values, terminal/tool/visible booleans, and bounded visible-length buckets. It never retains response text or reasoning and cannot hold, retry, rotate an account, apply cooldown, or mutate downstream bytes; a single bulk fragment is diagnostic evidence only.",
    ...
    "- Media/WS/models: Grok images/videos routes forward to `api.x.ai/v1`; image edits translate common OpenAI multipart uploads to xAI JSON data URLs; Responses GET bridges to `wss://api.x.ai/v1/responses`. Models, HTTP, media, and WebSocket inference routes require verified Router ingress with a Share identity; unsigned requests and signed client-lane requests without a Share are rejected. WS/image/edit/video/search/media-entitlement capabilities default fail-closed and use explicit current-generation evidence with TTL; legacy profile flags cannot authorize access. Video task ownership schema v4 keys Share/user/task kind/id plus Provider, exact Account, `authIdentityGeneration`, runtime fingerprint and upstream plane; v1-v3 migrate only to their historical video/xAI identity, and drift conflicts instead of resolving elsewhere. Model discovery uses a bounded ETag/TTL cache scoped to Provider/runtime plus Account auth/token generations, exposes `source`, `stale`, `fetchedAtMs` and a conservative model capability manifest, treats successful empty results as authoritative, permits stale only after network/408/429/5xx, and fails closed on 401/403, malformed success, binding drift or missing cache. Loopback WS/model endpoint injection exists only in test builds.",
