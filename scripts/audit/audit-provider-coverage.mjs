...
function serverEvidenceNotes() {
  return [
    ...
    "- HTTP usage records preserve the requested model, record the final upstream model and source, and attribute token usage to the final model. Direct Claude requests use only the configured current Provider and its bound account. Direct Codex requests use only the configured current or explicitly pinned Provider and, for Codex OAuth, the unique active account. Neither route enters cross-Provider or cross-account failover; Share requests additionally retain their immutable binding. Other routes retain their documented routing policies. Grok image/video routes are intentionally excluded.",
    "",
    ...
    "",
    "- Every non-`deleted` OAuth Share owns exactly one strong subscription identity. Claude identity is the normalized account UUID. Codex identity is the verified OpenAI subject paired with the effective verified workspace ID. Email, display name, local account row ID, token shape, and unverified workspace metadata are never identity fallbacks. A historical broken or stale Share still occupies every strong identity that can be derived from its bindings, so damage cannot be used to create a duplicate Share.",
    "- Ordinary Share upsert/import may update mutable Share fields but cannot change an existing binding. Provider binding changes use the dedicated binding endpoint, require `status=paused`, a matching Share `configRevision`, the same app, an unshared target Provider, and a resulting globally unique subscription identity. Codex workspace changes use the same paused/revision-CAS rule, advance the Account and Provider auth-identity generation together, and append a redacted identity fingerprint transition to `bindingHistory`.",
    ...
    "- A Share invocation always pins its stored Provider and its single bound subscription identity. It never enters account-pool selection, rotation, random choice, capacity fallback, cross-account failover, or cross-Provider failover. Same-account token refresh, same-Provider protocol/transport retries, and Codex WebSocket-to-HTTP fallback using the original execution identity are allowed.",
    "",
    "### OAuth login and cross-protocol bridge contract",
    "",
    "- `/api/accounts/capabilities` publishes ordered `loginFlows` entries (`browser_oauth`, `device_code`, `cli_manual_callback`) with callback/poll/cancel support for each account type. `supportsStartLogin` and `supportsCallback` remain backward-compatible fields derived from that list; they are not a second capability source. Claude OAuth exposes browser plus CLI-manual callback, while Codex OAuth additionally exposes device code.",
    ...
    "",
    "Claude OAuth protocol evidence review through 2026-07-26, implemented independently in Server:",
    "",
    ...
    "- Retry and semantic hardening: direct Claude requests resolve only `currentProviderClaude`; an explicit `x-cc-provider-id` must equal it, and Share requests retain their immutable Provider/account binding. Messages may replay once only for a connect-stage failure, once after a same-account forced refresh on 401, or through the bounded OAuth signature/thinking/web-search compatibility stages. Ambiguous send/timeout/body-read failures, 429/529, SSE error frames, and post-commit failures are never transparently replayed. `count_tokens` may retry bounded transport failures and one same-account 401 refresh because it is non-generating, but does not switch Provider/account. Native Anthropic JSON and SSE responses are incrementally validated for message/count shape, event ordering, terminal state, truncation, and bounded event/prelude size; downstream cancellation aborts the upstream body without penalizing Provider health.",
    ...
    "",
    "Codex/OpenAI OAuth protocol evidence review through 2026-07-26, implemented independently in Server. External references are protocol evidence only, not Share account-pool architecture templates:",
    "",
    ...
    "- Proxy headers/body: managed account requests finalize a paired official Codex identity (`originator`, configurable `version` defaulting to `0.144.1`, and User-Agent), inject the validated `chatgpt-account-id`, session/window headers, `reasoning.encrypted_content`, `prompt_cache_key`, and versioned instructions. With `store=false`, string references and object IDs carrying server-only `rs_`, `fc_`, `resp_`, or `msg_` prefixes are removed, as are `item_reference` entries. Top-level `reasoning_effort` is promoted to `reasoning.effort` with nested-value priority and model capability normalization. Cursor/Droid/Chat-compatible fields proven unsupported by Codex OAuth are removed without imposing a broader final allowlist. HTTP and WebSocket request frames use the same sanitizer. GPT-5.6 Sol/Terra/Luna capabilities and reasoning gates remain server-side registry data.",
    ...
    "- Active account and retry boundary: zero Codex OAuth accounts report `unconfigured`; one account is active automatically; multiple accounts report `needs_selection` until the administrator selects the unique proxy account. Selection atomically persists the active account and rebinds every unshared Codex OAuth Provider to that account/auth-identity generation; any Share that would be displaced rejects the operation before partial mutation. Direct traffic uses only the current or explicitly pinned Provider and the active account. HTTP, SSE, Images, models, alpha search, WS, and WS→HTTP fallback never enter candidate-account selection or cross-Provider/account failover. Codex 429 bodies parse `error.resets_in_seconds` and `error.resets_at`; generic managed-account handling honors bounded `Retry-After` but writes cooldown only to the active account.",
    ...
    "- Single-account retry boundary: every Grok Provider binds one concrete OAuth account. Initial managed-account load selection, HTTP/JSON, SSE, media, WebSocket handshake, and WS→HTTP fallback never rotate accounts or enter generic cross-Provider failover; same-account 401 handling may force-refresh only that account once. Untargeted `/v1/models` requests do not select an arbitrary Grok Provider. Credential persistence degradation returns 503 before Grok data-plane traffic.",
