...
function serverEvidenceNotes() {
  return [
    ...
    "",
    "Claude OAuth protocol evidence review from `/data/projects/proxy/Claude/Claude.md` and the 9router comparison through 2026-07-26, implemented independently in Server:",
    "",
    ...
    "",
    "Codex/OpenAI OAuth protocol evidence review from `/data/projects/proxy/pi`, `/data/projects/proxy/Codex/codex2api`, `/data/projects/proxy/Codex/Codex.md` v2 P0-P2, TokenRouter account-candidate filtering, and 9router Codex request/quota/image behavior through 2026-07-26, implemented independently in Server. These repositories are protocol evidence, not Share account-pool architecture templates:",
    "",
    ...
    "- Proxy headers/body: managed account requests finalize a paired official Codex identity (`originator`, configurable `version` defaulting to `0.144.1`, and User-Agent), inject the validated `chatgpt-account-id`, session/window headers, `reasoning.encrypted_content`, `prompt_cache_key`, and versioned instructions. With `store=false`, string references and object IDs carrying server-only `rs_`, `fc_`, `resp_`, or `msg_` prefixes are removed, as are `item_reference` entries. Top-level `reasoning_effort` is promoted to `reasoning.effort` with nested-value priority and model capability normalization. Cursor/Droid/Chat-compatible fields proven unsupported by Codex OAuth are removed without imposing 9router's broader final allowlist. HTTP and WebSocket request frames use the same sanitizer. GPT-5.6 Sol/Terra/Luna capabilities and reasoning gates remain server-side registry data.",
