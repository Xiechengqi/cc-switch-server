...
function serverEvidenceNotes() {
  return [
    ...
    "- Claude Messages and Codex Responses/Chat use fixture-verified Native Chat bridges with non-streaming and streaming tool lifecycles. The first eligible 401 may exchange and replay once on the exact same Account and identity generation; it cannot select another Account or Provider. Gemini-to-Copilot remains fallback and is not advertised as Native.",
    ...
    "",
    "Cursor OAuth/API key protocol evidence review through 2026-08-13, implemented independently in Server:",
    "",
    ...
    "- Proxy transport and isolation: Claude/Codex/Gemini Cursor providers use the native HTTP/2 Connect-RPC AgentService driver by default, with provider/env settings able to disable it during incident triage. The driver covers AgentService protobuf frames, cursor-agent CLI headers, KV/session handling, declared tools, images, and Anthropic/OpenAI Chat/OpenAI Responses/Gemini response formatting. Session, response, and tool-call indexes use a domain-separated typed scope containing App, Provider revision/runtime, exact OAuth Account plus auth/token generations or API-key digest plus credential generation, Share, and normalized signed user. Reused raw ids cannot be found, replaced, closed, or resumed across scopes. AgentService headers include W3C `traceparent`/`backend-traceparent`; timezone comes from `TZ`; client version is detected from local Cursor state with a 60-minute cache and falls back to `cli-2026.01.09-231024f`.",
    "- Stream and tool boundary: first business output has one absolute deadline starting when the request is sent; response headers, partial Connect bytes, KV/context/heartbeat/tool lifecycle, and unknown control frames neither satisfy nor extend it. A fresh absolute phase begins after tool results resume a parked stream, and only a complete business frame starts idle timing. Read/write/edit/delete/list/glob/grep, diagnostics, shell/background shell, fetch, stdin, and MCP requests bridge only to a compatible declared client tool or receive a protocol-correct rejection. The Server never executes arbitrary Cursor-requested filesystem or shell work.",
    ...
    "- Evidence: the Cursor-focused local suite passes 153 tests covering exact-scope sessions/indexes, initial and resumed absolute deadlines, builtin rejection, credentials/catalogs, all four response emitters, and signed Gemini Share catalog isolation. Maturity remains Experimental. Do not mark live Cursor OAuth/API-key proxy acceptance complete until a real Cursor account has exercised streaming, tool call/result continuation, images, builtin/declared tools, and rate-limit/cooldown behavior.",
    "",
