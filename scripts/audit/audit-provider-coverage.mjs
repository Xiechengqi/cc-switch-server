...
function serverEvidenceNotes(qoderVerification) {
  return [
    ...
    "",
    "Claude OAuth protocol evidence review through 2026-09-23, implemented independently in Server:",
    "",
    "- Proxy hot path: legacy-compatible and typed Claude OAuth Providers share one prepared-request contract: managed-account refresh, `?beta=true`, ordered model/body-driven betas, CLI/Stainless identity, fail-closed native/helper classification, global cache-control constraints, preserve-order JSON, and one final CCH over the cleaned body. The npm-latest 2.1.280 profile derives the billing suffix from the original first user text, keeps the billing block uncached, uses CCH seed `0x4D659218E32A3268`, and retains historical 2.1.258 vectors; active prompt/CCH goldens are `d7b`/`daec4`. A versioned capability registry drives Opus 5.5 discovery, adaptive/rejects-disabled reasoning conversion, mid-conversation, per-turn effort/timing and beta decisions. Nine 2.1.280 betas are model/body/explicit-request gated in audited order; ordinary OAuth requests do not synthesize fallback-credit. Unknown betas remain fail-closed. Tool-result cache markers are hoisted to the block, Responses string input is preserved, and hybrid MCP passthrough restoration requires a unique request-local match. Server fallback is never synthesized.",
    ...
    "- Wire profile and discovery: `assets/contract/claude-oauth-wire-profile.json` records a sanitized static audit of official npm latest Claude Code `2.1.280`, binary SHA-256, Stainless `0.112.1`, Node `v26.3.0`, Axios `1.15.2`, ordered beta rules and the Opus 5.5 capability catalog. Opus 5.5 publishes 1M context, 128K output, text/image input and five effort levels through the static model API; this metadata remains contract-verified while real subscription entitlement stays live-pending. Discovery performs zero OAuth/upstream requests. Fable 5/5.1 keep their existing single-account weekly capacity scope, and overage/Fable-only rejection ignores generic Retry-After rather than freezing unrelated capacity.",
