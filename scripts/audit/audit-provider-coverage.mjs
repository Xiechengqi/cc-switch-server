...
function serverEvidenceNotes() {
  return [
    ...
    "- Provider load performs only an in-memory compatibility normalization and never rewrites `providers.json`. An allowed explicit mode and actual model are preserved; missing non-native mappings use the Profile default, while typed official records are repaired to passthrough. Legacy model values are inferred from app configuration when possible, and legacy Grok providers without an explicit mapping default to `grok-4.6`. S1-to-S2 cutover is an explicit offline CLI action; unresolvable historical records block cutover rather than being guessed.",
