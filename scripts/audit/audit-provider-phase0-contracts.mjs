...
const adapterByAppType = Object.freeze({
  ...
  "claude:kiro_oauth": ["claude_kiro_codewhisperer", "native"],
  ...
  "codex:kiro_oauth": ["codex_to_kiro_anthropic", "native"],
...
const protocolByAppType = Object.freeze({
  ...
  "codex:kiro_oauth": "kiro_codewhisperer",
...
function buildWriterInventory() {
  ...
  return {
    ...
    entries: [
      ...
      writer(
        "bundle-sort",
        "src/api/providers.rs::update_provider_bundle_order|src/api/invoke/dispatch.rs::update_provider_bundles_sort_order",
        "REST and Web runtime writes delegate to update_provider_bundle_order_command, which validates one complete global Bundle order and persists ProviderStore.bundle_order through the shared commit coordinator without changing Surface revisions",
        "retain provider_commands::update_provider_bundle_order over ProviderStore::update_bundle_sort_order",
        "phase-3",
        "closed",
        "a per-Surface sort writer could split one multi-app Provider Bundle into conflicting Claude, Codex, and Gemini presentation orders",
        [
