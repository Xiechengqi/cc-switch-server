...
const contractPath = "docs/web-runtime-contract.json";
...
if (fs.existsSync(httpPath)) {
  ...
  const dispatchEnd = httpSource.indexOf("\nasync fn web_provider_quota(", dispatchStart);
  if (dispatchStart < 0 || dispatchEnd < 0 || dispatchEnd <= dispatchStart) {
    fail("src/http.rs must expose a parseable web_invoke_dispatch block");
  } else {
