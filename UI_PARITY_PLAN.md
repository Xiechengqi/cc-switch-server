...
### 2026-07-05 Settings auth/backup/diagnostics panels extraction

- 新增 `components/settings/SettingsAccountPanels.tsx`，抽出 `AuthSettingsPanel`、`BackupSettingsPanel`、`DiagnosticsSettingsPanel` 和局部 settings action/form footer helper，并从 settings barrel 导出。
- `SettingsPage.tsx` 继续持有 API token、owner email、backup reason、restore confirm、busy/result/error 与 action 编排；认证、备份和诊断 tab 的具体 UI 由新文件承接。
- 保留 API token rotate/copy、email code request/verify、AuthCenter embedded、backup create/restore、diagnostics summary/cards 的 DOM、CSS class、i18n 文案、busy key 和所有 API 调用；不改变 settings/auth/backup/diagnostics payload 或行为。

### 2026-07-05 Share export modal extraction

- 新增 `components/share/ShareExportModal.tsx`，抽出 Share 导出 fallback textarea、Copy JSON 重试按钮和复制状态展示，并从 share barrel 导出。
- `SharePage.tsx` 继续持有 export text、clipboard copy、busy/result/error 与 export API action 编排；导出 fallback modal 的 UI 由新文件承接。
- `providerKey` 与 `shareActionLabel` 归入 `components/share/shareDisplay.ts`，并清理 SharePage 中不再使用的 lucide/IconAction/SimpleModal imports；不改变 share export/import JSON、clipboard feedback、action result 或任何 share API 调用。

### 2026-07-05 Usage tabs/state extraction

- 新增 `components/usage/UsageTabs.tsx`，抽出 Usage tab strip 与 tab button，并从 usage barrel 导出 `UsageTabs` / `UsageTab`。
- 新增 `components/usage/usageState.ts`，集中默认 filter draft、filter-to-API 转换、provider limit 前端过滤、empty rollup、data source breakdown 和 error helper。
- `UsageDashboard.tsx` 从约 517 行降至约 350 行，只保留 usage 数据加载、pricing/backfill 写操作、modal confirm 编排和页面组合；保留 tab DOM/CSS class、filter query 语义、data source aggregation、ProviderCard focus、pricing/backfill confirm 与所有 usage API 调用。

### 2026-07-05 ProviderList import boundary cleanup

- 清理 `ProviderList.tsx` 中 ProviderCard/Form/ConfirmDialog/IconPicker/JsonEditor 等抽离后遗留的 import-only 依赖，保留仍由页面编排使用的 dnd、provider action 和 state 类型。
- 本切片不改变 ProviderList 渲染、DND 排序、catalog/form modal、failover queue、model fetch、switch/delete/test 或任何 provider API 调用；只收紧已完成组件抽离后的文件边界。

### 2026-07-05 Universal export modal extraction

- `components/universal/UniversalModals.tsx` 新增 `UniversalExportModal`，承接 Universal 导出 fallback textarea、Copy JSON 重试按钮和复制状态展示，并从 universal barrel 导出。
- `UniversalProviderPanel.tsx` 继续持有 export text、clipboard copy、busy/result/error 与 export API action 编排；导出 fallback modal 的 UI 由 modals 文件承接。
- 同步清理 UniversalProviderPanel 中由 UniversalCard/Form/Modal 抽离后遗留的 dnd card helper、IconAction、JsonPreview、StatusPill、SimpleModal 等 import-only 依赖；不改变 universal provider export/import/sync/save/delete 或任何 schema/API 行为。

### 2026-07-05 非验证实施收口审计

- U1-U10 的可实施代码项已完成到 server-safe parity 边界：旧 Phase L dashboard 入口、summary/matrix/table 默认展示、页面大标题、独立 Accounts 入口、浏览器原生 confirm、宽表格和主要 inline 巨型组件均已替换或抽离。
- 剩余未标为代码任务的差异集中在 U11：人工截图核对、四语言词条级审读、亮/暗主题目视核对、移动/桌面宽度核对和逐像素行为差异记录。按用户约束禁止 UI 自动化，因此不在代码实施轮次中继续推进。
- 已明确豁免继续实施的 desktop-only/excluded 项：Tauri window/useAutoCompact 行为、desktop live proxy takeover 写操作、MCP/skills/sessions/OpenClaw/Hermes/OMO 等 token server 不需要的功能、MB 级 SVG 图标，以及会突破 server binary 内嵌 web-dist 体积门禁的 CodeMirror/Recharts 原样替换。
- 当前可继续做的只剩人工验证后根据 checklist 发现的具体缺口；在没有人工核对新证据前，不再把“更深 desktop 组件复用”作为开放代码任务，避免无边界重写。
