...
use crate::core::router_client::{ShareAppAccess, ShareAppSettings, ShareSettingsPatch};
...
impl ShareStore {
    ...
    pub fn transfer_owner_verified(
        &mut self,
        share_id: &str,
        ...
        let share = self
            ...
            .find(|item| item.id == share_id)
            .ok_or(SharePatchError::NotFound)?;
        let new_owner_email = normalize_verified_email(&new_owner_email)?;
        let old_owner = share.owner_email.clone();
        if old_owner.as_deref() != Some(new_owner_email.as_str()) {
            ...
            share
                .acl
                .shared_with_emails
                .retain(|email| !email.eq_ignore_ascii_case(&new_owner_email));
            share.owner_email = Some(new_owner_email.clone());
            if let Some(snapshot) = share.runtime_snapshot.as_mut() {
                if let Some(object) = snapshot.as_object_mut() {
                    object.insert("ownerEmail".to_string(), json!(new_owner_email));
                }
            }
        }
        Ok(share.clone())
...
fn runtime_snapshot_for_share(
    ...
    let health = provider.map(|item| crate::core::health::provider_health(item, usage));
...
mod tests {
    ...
    fn applies_share_market_app_settings_patch() {
        ...
        let descriptor =
            crate::core::router_client::descriptor_for_share(&share, &ProviderStore::default());
        assert_eq!(
            descriptor
                .app_settings
                .get("codex")
