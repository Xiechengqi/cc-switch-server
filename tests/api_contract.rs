...
#[tokio::test]
async fn provider_settings_web_invoke_use_seconds_validate_and_persist() {
    let state = test_state();
    let config_dir = state.config_dir.clone();
    let app = app_router(state);
    let token = setup_and_login(&app).await;

    let request_defaults = json_body(
        app.clone()
            .oneshot(json_request(
                Method::POST,
                "/web-api/invoke/get_provider_request_defaults",
                json!({}),
                Some(&token),
            ))
            .await
    ...
    .await;
    assert_eq!(request_defaults["requestTimeoutSeconds"], 300);
    assert_eq!(request_defaults["streamFirstByteTimeoutSeconds"], 120);
    assert_eq!(request_defaults["streamIdleTimeoutSeconds"], 300);
    assert!(request_defaults.get("timeoutMs").is_none());

    let health_defaults = json_body(
        app.clone()
            .oneshot(json_request(
                Method::POST,
                "/web-api/invoke/get_provider_health_check_config",
                json!({}),
                Some(&token),
            ))
            .await
    ...
    .await;
    assert_eq!(health_defaults["timeoutSeconds"], 45);
    assert_eq!(health_defaults["degradedThresholdSeconds"], 6);
    assert_eq!(health_defaults["testModels"]["codex"], "gpt-5.6-luna@low");
    assert!(health_defaults.get("degradedThresholdMs").is_none());

    let saved_request = app
        .clone()
        .oneshot(json_request(
            Method::POST,
            "/web-api/invoke/save_provider_request_defaults",
            json!({
                "defaults": {
        ...
        .await
        .unwrap();
    assert_eq!(saved_request.status(), StatusCode::OK);

    let saved_health = app
        .clone()
        .oneshot(json_request(
            Method::POST,
            "/web-api/invoke/save_provider_health_check_config",
            json!({
                "config": {
                    "timeoutSeconds": 20,
                    "maxRetries": 1,
        ...
        .await
        .unwrap();
    assert_eq!(saved_health.status(), StatusCode::OK);

    let persisted: Value =
        serde_json::from_str(&std::fs::read_to_string(config_dir.join("server.json")).unwrap())
            .unwrap();
    assert_eq!(
        persisted["providerRequestDefaults"]["requestTimeoutSeconds"],
        90
    );
    assert_eq!(
        persisted["providerRequestDefaults"]["streamFirstByteTimeoutSeconds"],
        30
    );
    assert_eq!(
        persisted["providerHealthCheck"]["degradedThresholdSeconds"],
        4
    );
    assert_eq!(
        persisted["providerHealthCheck"]["testModels"]["codex"],
        "gpt-5.6-luna@low"
    );

    let invalid_request = app
        .clone()
        .oneshot(json_request(
            Method::POST,
            "/web-api/invoke/save_provider_request_defaults",
    ...
    let invalid_health = app
        ...
        .await
        .unwrap();
    assert_eq!(invalid_health.status(), StatusCode::BAD_REQUEST);
}
...
async fn web_invoke_email_auth_owner_change_updates_config_and_shares() {
    ...
    let router = Router::new()
        .route(
            ...
            post(
                |State(change_seen): State<Arc<AtomicUsize>>,
                 headers: HeaderMap,
                 axum::Json(body): axum::Json<Value>| async move {
                    assert!(
                        headers.get(axum::http::header::AUTHORIZATION).is_none(),
                        "installation-signed owner change must not require bearer token"
                    );
