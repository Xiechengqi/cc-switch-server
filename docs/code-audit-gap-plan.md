...
> **与 Phase R 的关系**：Phase R 已关闭（2026-07-07）。X4–X11 文中引用的 `src/http.rs`、`src/core/*` 旧路径按 R2/R3 映射表对应到 `src/api/*`、`src/domain/*`、`src/clients/*`；R4 剩余的存储收敛（32 处直接写）与 `api/types.rs` DTO 就近化随 X 系列功能 PR 摊销，accounts/pricing/universal_providers/sessions/oauth_logins 域已完成并解除 X5 前置。
