...
- **状态（2026-07-07）**：**第一/二/三批静态实现已完成**。新增 `src/proxy/kiro.rs`，server-native 移植 desktop `kiro_claude.rs` 的 Claude Messages → Kiro conversation payload、Kiro event stream → Claude SSE/JSON、tool leak rescue、thinking/redacted thinking、usage/prompt-cache 计算等纯协议逻辑；desktop 23 个 fixture 已全部进入 server 并通过。第二批补齐 server `Account` → `KiroAccountData` → `KiroPreparedRequest`（URL/header/body/tool map）转换，覆盖 device-flow import 写入的 profile/raw 字段和 CodeWhisperer header shape。第三批在 forwarder 中为 Claude + `KiroOAuth` 增加 managed-account 专用发送路径：请求走代理感知 client，输出 Kiro CodeWhisperer payload/header，非流式 eventstream 聚合为 Claude JSON，流式 eventstream 转 Claude SSE，并记录 usage/share/failover outcome；API 合同测试以 mock CodeWhisperer 覆盖非流式与流式。Codex/Gemini Kiro 入口、账号 refresh 完整真实验收和 capability 矩阵升级仍保持 gate，避免未真实账号验收前误报 native。
