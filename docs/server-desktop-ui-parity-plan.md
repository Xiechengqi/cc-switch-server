...
> **关联文档**：`docs/web-runtime-contract.json`（功能边界）、`docs/manual-ui-checklist.md`（人工验收门禁）。本地-only 历史笔记（不提交）：`UI_PARITY_PLAN.md`、`DESKTOP_ALIGNMENT_TASKS.md`、`docs/remaining-work-index.md`。
...
| 决策项 | 结论 | 影响 |
| --- | --- | --- |
| Embedded 体积预算 | **接受总量从 900KB 提升至约 1.5–2MB** | 可引入 CodeMirror `JsonEditor`、recharts `UsageDashboard`、完整四语言 locale chunk |
| Server 扩展能力归属 | **全部放入 Settings**，不单独占顶栏一级视图 | Usage / Universal / Router / Tunnel / Failover / Diagnostics 均在 Settings tab 内 |
...
依据 `docs/web-runtime-contract.json`：

- Providers（Claude Code / Codex / Gemini）
- Shares
- Usage & Pricing
- Settings（含 Auth、Router、Backup、Failover、Tunnel、Diagnostics）
- Universal Providers
- Accounts / OAuth / Quota（归属 Settings → Auth）
- Proxy 状态、Failover 状态（顶栏只读展示 + Settings 配置）
...
| U2 | Provider 列表降噪、dnd、health、failover badge | 代码收口，待 U11 |
| U3 | Catalog modal、轻量表单、轻量 JsonEditor | 代码收口，待 U11 |
| U4–U8 | Settings/Share/Usage/Universal/Auth 首切 | 代码收口，待 U11 |
...
| Provider 表单 | `providers/forms/**`（68 文件） | `ProviderFormModal` 手写 | **未同源** |
...
| 项 | 变更 |
| --- | --- |
| 体积门禁 | `scripts/audit-web-dist-size.mjs` 默认上限 **2MB**（`2097152`） |
...
| 导航 IA | Usage → Settings/Usage tab；Universal → Settings/Universal tab；顶栏仅 providers/shares/settings |
| 同步工具 | `scripts/sync-desktop-ui.mjs` |

当前构建体积约 **1.56MB**（`locales` 598KB + 主 JS 831KB + CSS 116KB）。
...
| Universal（AddProviderDialog tab，无顶栏入口） | Settings/universal tab（管理入口）；添加流程仍走 AddProviderDialog |
...
| proxy | desktop | 上游代理；顶栏 Proxy 只读态跳转此 tab |
| failover | server | per-app 开关、threshold、breaker 摘要 |
...
| importExport | desktop | providers/shares/universal JSON |
| usage | desktop | 嵌入 desktop `UsageDashboard` |
| universal | desktop | 嵌入 desktop `UniversalProviderPanel` |
...
U12.3 App 壳原样移植    U12.4 Settings 原样移植 + server tab
    ↓                        ↓
U12.5 Share / U12.6 Usage / U12.7 Universal（并行）
    ↓
...
| 体积 | `node scripts/audit-web-dist-size.mjs`（< 2MB） |
...
#### 同步清单（`scripts/sync-desktop-ui.mjs`）
...
components/providers/forms/**          # 68 文件
...
| ID | 任务 | 验收标准 | 工作量 |
| --- | --- | --- | --- |
| U12.2a | 运行 sync + 修编译 | `npm run typecheck` 通过；无 Tauri import 泄漏 | L |
| U12.2b | Runtime shim 层 | `lib/server-shims.ts`：`openExternal`、`pick_directory` 等 excluded command 有 web 实现或安全 no-op | M |
| U12.2c | ProviderList/Card 接线 | 列表、dnd、当前 provider 高亮、hover actions、9 种 QuotaFooter 按类型渲染 | L |
| U12.2d | Add/Edit Dialog 接线 | preset 卡片、per-app 表单分区、OAuth section、partner badge 与 desktop 一致 | XL |
| U12.2e | 删除 server-local 替代品 | 移除 `ProviderFormModal.tsx`、`ProviderFormSections.tsx`、`ProviderCatalogModal.tsx`（由 desktop 组件替代） | M |
| U12.2f | Server 诊断字段 | adapter readiness 等 server 字段放入 desktop `ProviderAdvancedConfig` 折叠区，不改变默认布局 | M |

**依赖**：U12.1a  
**风险**：表单 hooks 依赖 desktop `lib/api` / `lib/query` 形状——需在 `web-src/src/lib/api.ts` 补齐或 adapter 映射。
...
| U12.3c | FailoverToggle | `components/proxy/FailoverToggle.tsx` | 接 `/api/failover` | 与 desktop 交互一致 | M |
...
| U12.4b | 并入 server tab | failover/router/tunnel/directory/diagnostics/importExport 用相同 `TabsTrigger`/`TabsContent` 样式 | M |
| U12.4c | 移植子面板 | `LanguageSettings`、`ThemeSettings`、`GlobalProxySettings`、`ImportExportSection`、`BackupListSection`、`AuthCenterPanel`、`AboutSection` | L |
| U12.4d | 嵌入 Usage/Universal | Settings/usage → desktop `UsageDashboard`；Settings/universal → desktop `UniversalProviderPanel` | M |
| U12.4e | 删除 server-local Settings 分片 | 移除 `SettingsAccountPanels.tsx` 等过渡实现（逻辑迁入移植组件或薄 wrapper） | L |

**依赖**：U12.6、U12.7（嵌入面板可先占位后替换）
...
| ID | 任务 | 验收标准 | 工作量 |
| --- | --- | --- | --- |
| U12.7a | 同步 `components/universal/**` | 卡片、dnd、preset modal、Save and Sync 与 desktop 一致 | M |
| U12.7b | Settings 嵌入 | 仅通过 Settings/universal tab 管理；AddProviderDialog universal tab 用于创建 | S |
...
| U12.9b | locale 随 desktop 同步 | `scripts/sync-desktop-ui.mjs` 含 `i18n/locales/*.json` |
| U12.9c | 审计 | `scripts/audit-web-i18n-literals.mjs` 保持 0；四语言人工审读 |
...
- [ ] excluded 功能不可见
- [ ] `scripts/import-desktop-export.mjs` 导入后 UI 展示一致
...
所有 retained UI 调用的 command 必须在 `docs/web-runtime-contract.json` 登记，并通过：

```bash
node scripts/audit-web-runtime-contract.mjs --check
```
...
| 项 | 值 |
| --- | --- |
| 默认上限 | **2MB**（`CC_SWITCH_WEB_DIST_MAX_BYTES=2097152`） |
| 当前基线 | ~1.56MB（U12.0 后） |
| 审计脚本 | `scripts/audit-web-dist-size.mjs` |
...
# 仅同步 Provider 表单
node scripts/sync-desktop-ui.mjs components/providers/forms

# 检查 server 是否落后于 desktop
node scripts/sync-desktop-ui.mjs --check
```

默认同步路径见 `scripts/sync-desktop-ui.mjs` 内 `defaultPaths`。

### 9.2 Desktop 上游吸收流程

每次 desktop UI 有提交时：

1. 运行 `node scripts/sync-desktop-ui.mjs --check`
...
| Provider 矩阵 | `node scripts/audit-ui-provider-matrix.mjs --check` |
| 契约 | `node scripts/audit-web-runtime-contract.mjs --check` |
| 体积 | `node scripts/audit-web-dist-size.mjs` |
| 本地冒烟 | `scripts/smoke-local.sh`（需允许启动 server） |
...
| U12.5–U12.7 | Share/Usage/Universal | 4–5 天（可并行） |
...
| 体积超 2MB | 懒加载 CodeMirror/recharts；locale 可按语言动态 import |
...
1. **U12.2 Provider 表单整包移植**（用户感知最大）
2. **U12.3 App 壳**（导航与顶栏）
3. **U12.4 Settings + U12.6 Usage**（Usage 进 Settings + recharts）
4. **U12.5 Share + U12.7 Universal**
...
| `docs/web-runtime-contract.json` | retained/excluded 功能契约 |
| `docs/manual-ui-checklist.md` | 人工验收清单 |
| `AGENTS.md` | 仓库开发约定 |
| `scripts/sync-desktop-ui.mjs` | Desktop → Server UI 同步 |
