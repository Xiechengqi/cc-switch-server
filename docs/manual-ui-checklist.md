...
- Request, first-byte, and stream-idle timeouts resolve field by field as Provider override → Server default. They appear once at Bundle scope, never in Surface tabs; blank inputs preserve inheritance and show the Server values as light placeholders that disappear on focus.
...
- Settings → Advanced → Provider Runtime Defaults persists the three timeout defaults and Claude/Codex/Gemini test-model defaults in `server.json`; saving recompiles runtime plans without materializing inherited values into Provider records.
- Settings → Advanced → Model Test Config contains probe timing, retry, degradation-threshold, and prompt settings only. The saved prompt is the content sent by manual and scheduled real-model probes.
