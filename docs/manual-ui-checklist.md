...
- Each Surface tab contains an independent model policy only in Per-App mode. Otherwise it contains only that App's enable state, custom endpoint override when the Family permits it, protocol/auth configuration when custom binding permits it, Surface credential slots and headers, and the read-only effective runtime diagnostic.
