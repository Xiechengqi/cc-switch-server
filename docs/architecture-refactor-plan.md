...
| 项 | 状态与后续 |
| --- | --- |
| R4 存储收敛 | 已完成 shares/usage/config/accounts/failover/providers/pricing/universal_providers/sessions/oauth_logins 十个域（api 层零直接写；accounts 已通过 state 域方法封装立即保存与 debounce 保存，failover 已封装控制面立即保存与 proxy 热路径 best-effort 状态保存，providers/pricing/universal_providers 已通过 state 域方法封装立即保存或按变更保存，sessions/oauth_logins 已通过 state 域方法封装内存会话写入；对应字段降为 `pub(crate)`）。剩余 1 个域共 14 处直接写：ui_settings 14。随功能 PR 摊销，每收敛一域将对应 state 字段降 `pub(crate)`。**X5 前置已满足：Copilot token 交换可在 accounts 域方法上新增后台并发写路径，不得重新引入 state 外直接写**。`api/types.rs`（1295 行 / 127 个 DTO）的域专属 DTO 随各域收敛时就近搬到对应 handler 文件，`types.rs` 只保留跨域共享类型 |
| R6 workspace 化 | 挂起，三个触发条件（增量编译变慢 / 第二交付物 / 多人并行）均未满足；R2/R3 目录边界即未来 crate 边界 |

后续工作回到功能主线 `docs/code-audit-gap-plan.md`：P0 已清零；X4/X5/X7/X8 第一批/X9/X10/X11 已静态落地，剩余本地可实施项以 X6 Kiro 桥、X8 后续 CSS 收口和 R4 剩余域收敛为主，真实发信/OAuth/router/market 验收仍按外部环境 gate 推进。
