...
后续工作回到功能主线 `docs/code-audit-gap-plan.md`：P0 已清零；X4/X5/X6 第一/二批/X7/X8 第一批/X9/X10/X11 已静态落地，R4 存储写路径收敛已完成。剩余本地可实施项以 X6 Kiro forwarder 发送/响应桥接、X8 后续 CSS 收口和 `api/types.rs` DTO 就近化为主，真实发信/OAuth/router/market 验收仍按外部环境 gate 推进。
