...
- `scripts/` 20+ 脚本平铺。
- `web-src/` 曾缺少明确的 Server 产品边界；现由 Server-owned runtime contract、Provider registry 和产品边界审计共同约束，不依赖任何外部 UI 文件树。
...
- `web-src/` 前端按 Server 产品边界独立维护；目录重组必须保持 Server API、runtime contract、可访问性和人工验收行为不变。
...
| R3.5 | 按域抽出 REST handler：`api/providers.rs`（provider handler + test/fetch-models）、`api/accounts.rs`（accounts/device flow/login）、`api/shares.rs`（share CRUD/binding/ACL/market/connect-info/tunnel）、`api/usage.rs`（Token usage/quota/Share limits）、`api/router.rs`（register/heartbeat/status/batch-sync/client-tunnel/diagnostics/share-edits）、`api/settings.rs`（config/setup/auth）、`api/backup.rs` | 用 `rg -n 'async fn ' src/api/mod.rs` 生成清单，按路由表分组归属；跨域共享的小工具函数（如 `now_ms`、序列化辅助）集中到 `api/mod.rs` 或 `api/util.rs`，禁止复制多份 |
...
- **R5.4 Web 产品边界显式化**：以 `assets/contract/web-runtime-contract.json` 约束命令和功能，以 `audit-server-product-boundary.mjs` 阻止已排除的客户端功能、外部 UI 同步工具和同步清单重新进入 production graph。Web 组件、样式和 locale 均由 Server 自己维护。

验收：R5.1 后 `cargo test` + `static-checks` 全绿且 `rg 'include_str!\("../docs' src` 零命中；R5.2 后 `cargo test` 全绿；R5.3 后 `scripts/static-checks.sh`（新路径）与 CI workflow 语法通过；R5.4 后产品边界单测和 `audit-server-product-boundary.mjs` 均通过，并能拒绝重新加入已删除的 UI 同步资产。
...
### R8 `proxy/` 体积拆分（挂起项，不在本轮执行）

**背景（2026-07-07 登记）**：Phase R 红线要求重构期间「不动 proxy」，但 X6/Y 系列增长几乎全部落在 `proxy/`。当前全仓最大四个源文件中三个在 proxy：

| 文件 | 行数（约） | 备注 |
| --- | ---: | --- |
| `proxy/adapters.rs` | 5753 | 生产代码 ~2990 行，已超 R3 给 `api/` 定的单文件 2500 行红线 |
| `proxy/transforms.rs` | 4368 | transform + fixture 宏 |
| `proxy/kiro.rs` | 3274 | X6 协议桥 |

Kiro Codex/Gemini 路径、DeepSeek 剩余路径仍会继续往上述文件写入；**现在拆分必然返工**。

**目标形态（待触发后细化）**：按 app 或 provider 族拆分 `adapters.rs`（如 `adapters/claude.rs`、`adapters/codex.rs`、`adapters/gemini.rs` 或按 managed-account 族分组）；fixture 测试抽到对应子模块 `#[cfg(test)]`；`transforms.rs` / `kiro.rs` 按同类边界评估是否并行拆分。

**触发条件（须全部满足，启动前在本文档登记决策）**：
1. X6 全部落地（含 Kiro Codex/Gemini、DeepSeek 剩余路径静态接线完成）；
2. proxy 侧新增 provider 族进入平台期（连续两个迭代无大块新增转发路径）；
3. `adapters.rs` 生产代码稳定超过 3500 行或增量编译/审查成本成为瓶颈。

**验收（触发后）**：拆分后无单文件超 2500 行生产代码；`cargo test` 数量不减少；`scripts/static-checks.sh`（含 clippy）全绿；capability 矩阵与 fixture 路径无漂移。
...
- 归类依据：该模块读取 legacy 配置文件并产出 `Provider` 领域对象、读写 ui_settings 的 current-provider 键——是**领域导入服务**；其本地 fs 读取与各 store 的 `load_or_default` 同性质，不违反 domain 纯度（2.1 节的"纯"指无网络出站）。
...
**Phase R 正式关闭**：R1–R7 已全部实施并经独立复核（提交 `65721b8`）。关闭时验证快照：`cargo test` 503 passed（484 lib + 19 集成）、typecheck 0、`static-checks.sh` 0（含 5 条行内模式方向门禁）、`rg 'crate::(api|clients|domain|proxy)' src/infra` 零命中、`rg 'crate::core' src` 零命中。2026-07-22 起 Web 改由 Server-owned contract 和产品边界审计独立约束。

**R4 正式完结（2026-07-07 复核）**：十个受管 store 域（config/providers/universal_providers/accounts/failover/pricing/usage/shares/ui_settings/sessions/oauth_logins）生产写路径已全部收敛到 `ServerStateInner` 域方法；字段均为 `pub(crate)`；`static-checks.sh` 状态写入门禁覆盖多行写锁与直接 `save_*` 调用。当前验证基线 **`cargo test` 595 passed（565 lib + 30 集成）**。

计划内仍开放的两项（均为设计上的渐进/挂起项，不阻塞关闭）：

| 项 | 状态与后续 |
| --- | --- |
| R4 存储收敛 | **已完结**。生产写路径已全部收敛到 `ServerStateInner` 域方法，受管 store 字段均为 `pub(crate)`，静态门禁覆盖多行写锁与直接 `save_*` 调用。**DTO 收口已完成**：原 `api/types.rs`（1299 行）拆为 `api/types/{common,auth,backup,settings,providers,accounts,usage,shares,router,models}.rs`，`mod.rs` 仅 re-export；跨域共享类型保留在 `common.rs`。历史关闭登记中的「剩余 59 处直接写」为渐进收敛过程中的中间态，已于 R4-ui_settings 完成后清零 |
...
| 2026-07-07 | R4 DTO 收口：`api/types/` 域子模块拆分完成；X8 第二批 CSS 过渡层 1917 行（门禁 2000） |
| 2026-07-07 | **R4 正式完结**：十个受管 store 域写路径全部收敛、静态门禁入 `static-checks.sh`；历史「剩余 59 处直接写」为关闭登记时的中间态，已清零；当前验证基线 595 passed（565 lib + 30 集成） |
| 2026-07-07 | clippy 清零并纳入 `static-checks.sh`（`-D warnings`）；`api/types/mod.rs` glob re-export 改为 `pub(in crate::api) use` 与子模块 DTO 可见性对齐 |
| 2026-07-07 | **R8 挂起登记**：proxy 三巨文件（adapters/transforms/kiro）待 X6 全部落地且增长停稳后按 app/provider 族拆分 |
