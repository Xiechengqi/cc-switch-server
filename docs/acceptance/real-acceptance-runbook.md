...
| `CC_SWITCH_QODER_<RAIL>_SHARE_ID` | 对应 rail 的显式 Share；必须同时固定三个 Surface Provider | receipt 只记录 identity/binding digest 与 revision |
...
1. Global Device OAuth、Global PAT、CN Device OAuth 必须分三次独立运行，`CC_SWITCH_QODER_REAL_RAIL` 分别设为 `global_oauth`、`global_pat`、`cn_oauth`。每次只配置该 rail 的 Account selector、Share 和 Claude/Codex/Gemini 三个 Provider ID；三个 Provider 必须为 `special.qoder_cosy` / `ready`，固定同一 `qoder_cosy` Account 和同一 `authIdentityGeneration`，Share 必须显式固定这三项绑定。另一账号、另一 rail、另一 site、Share 与 decoy Provider 不得参与。
...
6. `QODER_REAL_RECEIPT_FILE` 必须是仓库外、父目录已存在、目标文件尚不存在的绝对路径。schema-v2 receipt 绑定完整 target commit、harness revision、site/rail、exact model、Account auth/token generation、三个 Surface Provider revision/runtime digest、Share revision/binding digest、三份 fresh catalog snapshot 与六个请求 body hash；另保存 quota、Surface/terminal、恢复决策、敏感扫描和三个 decoy 计数。禁止保存 token、PAT、prompt、callback、raw request/response/body 或可逆 Account/Provider/Share ID。
...
5a. 对新鲜、无冲突且明确为 Max 5x/20x、但没有活动 Fable observation 的账号，账号 quota、Provider UI 与 Router Share descriptor 应显示 `Fable 7d · 待观测`：只能出现在 `unobservedTiers`，不能生成 utilization/reset 或 `0%` tier，也不能改变 Provider/模型可用性。核对刷新过程没有为了获取该数值发送 Fable 推理或探测请求；stale/conflict/generic Max 和明确 Free/Pro/Team/Enterprise 不得出现提示。随后在明确绑定 Max 20x 账号的 Fable 5.1 non-stream 与 SSE 请求上，记录脱敏后的 `5h`/`7d`/`7d_oi` header 存在性、取整 utilization 和 reset 倒计时范围，不保存整组原始 header。刷新账号/Provider quota 后必须出现真实 `Fable 7d` tier 并覆盖提示，且后端 tier metadata 为 `scope=model_family`、`capacityPool=claude_fable_7d_oi`、`modelFamily=claude-fable-5`、`relativeWeeklyCapacity=0.5`、`source=anthropic_ratelimit_7d_oi`，`queriedAt` 不早于样本时间。主动 usage 若已提供同名 tier，必须优先于被动样本；样本到期且计划仍新鲜时应回退待观测。若真实响应没有 `7d_oi` 证据，数值验收保持 `live_pending`，不得从 7d 或计划倍率推测。
...
24. Claude 真实验收分成 `oauth_inference`、`max_5x_plan`、`max_20x_plan`、`fable_5_1` 四个互不替代的 operation。每项必须显式提供同一 operation 的 Account selector、`claude_oauth` Provider ID、Share ID、model 和仓库外私有 receipt 路径；Provider 必须是 `oauth.claude_messages` 且绑定该 Account 的当前 `authIdentityGeneration`，Share 必须明确绑定该 Provider。禁止用另一个 Account、Provider、Share、计划等级或模型的成功覆盖当前 operation。
25. `node scripts/smoke/claude-oauth-real.mjs` 只做低成本 count_tokens、Messages 和计划解析 probe，输出不构成 live acceptance。完成真实请求、401/429/terminal/decoy 检查并生成脱敏 receipt 后，使用 `RUN_REAL=1 node scripts/smoke/claude-real-receipt.mjs --operation <operation>` 验证。validator 会把 receipt 绑定当前 Git commit、Account/token generation、Provider revision/runtime fingerprint、Share revision、精确模型和可选 fresh plan projection；receipt 必须是仓库外绝对路径且权限为 `0600`。
