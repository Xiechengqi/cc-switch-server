...
上游固定为 `https://api.kimi.com/coding/v1`，使用 OpenAI Chat 协议。最终身份头包括 `KimiCLI/1.37.0`、`X-Msh-Platform: kimi_cli`、CLI version、设备名、设备型号、OS 版本和设备 ID。

账号 `extraHeaders` 不能覆盖 Authorization、User-Agent 或任何 `X-Msh-*` 设备身份头。最终 Kimi 身份在通用账号 header 合并后重新覆盖，避免历史配置改变认证设备。

模型采用 fail-closed allowlist：
...
- `/v1/models` 只在当前 Share 选中的 Kimi Provider 范围内公开这些 aliases，不参与 Provider 或账号选择。

## 验收边界

离线测试覆盖 device poll 串行化、slow-down、稳定设备身份、JWT principal、账号 Profile round-trip、header 最终覆盖和模型 allowlist。真实 Kimi Code 账号仍需分别验证：
