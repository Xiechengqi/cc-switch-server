...
Server-native Claude OAuth proxy parity pass from `/data/projects/proxy/Claude/CLAUDE.md` P0/P1 plus second/third/fourth-round hardening (2026-07-09):
...
- Operations hardening: the quota refresh loop first warm-refreshes due native OAuth tokens, Claude OAuth non-streaming version-gate responses are rewritten into admin-facing guidance to bump `CC_SWITCH_CLI_UA_VERSION` / `CC_SWITCH_CLI_UA`, and upstream `x-request-id` is passed through for support correlation.
