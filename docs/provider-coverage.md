...
Server-native Claude OAuth proxy parity pass from `/data/projects/proxy/Claude/CLAUDE.md` P0/P1 plus subsequent hardening through 2026-07-13:
...
- Usage/transform semantics: internal usage is normalized to four non-overlapping buckets—fresh input, cache read, cache creation, and output. Anthropic input is treated as cache-exclusive; OpenAI Chat/Responses and Gemini input is cache-inclusive. Streaming and non-streaming paths use the same explicit semantics, and Responses/Chat/Anthropic bridges preserve `cache_creation_input_tokens` in both directions without double subtraction. The existing router payload fields remain unchanged.
...
- OAuth web-paste/profile: `code#state` parsing, platform token endpoint first, platform User-Agent (`axios/1.13.6`). OAuth exchange performs a non-blocking `/api/claude_cli/bootstrap` lookup, and quota refresh repeats it in parallel with usage/profile work. Only normalized account UUID/email, organization UUID/name/type/rate-limit tier, and refresh timestamp are merged into account profile; bootstrap failure does not invalidate a usable token.
- Beta/session hardening: Claude OAuth accepts client `anthropic-beta` values only from the protocol-owned set or the exact audited compatibility tokens `prompt-caching-2024-07-31` and `token-efficient-tools-2025-02-19`; unknown experimental tokens are dropped with debug-level diagnostics. OAuth login sessions can be cancelled atomically before exchange, cancellation is idempotent and terminal, completed sessions retain the imported account id for idempotent multi-tab completion, and unknown states remain rejected. Cancellation is intentionally rejected after token exchange starts so an already-consumed authorization code cannot be left without a persisted account.
...
Server-native Codex/OpenAI OAuth proxy parity pass from `/data/projects/proxy/Codex/Codex.md` v2 P0-P2 through 2026-07-13:
