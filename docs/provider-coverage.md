...
- HTTP usage records preserve the requested model, record the final upstream model and source, and attribute token usage to the final model. Direct Claude requests use only the configured current Provider and its bound account; Share requests use only their immutable binding. Claude never enters cross-Provider or cross-account failover. Non-Claude routes retain their documented routing policies. Grok image/video routes are intentionally excluded.
...
- Rate limits/account dispatch: Codex 429 bodies parse `error.resets_in_seconds` and `error.resets_at`; generic managed-account handling also honors `Retry-After`, bounds cooldowns, and writes only account `rateLimitedUntil`. Non-Share unpinned requests retain the existing direct-routing candidate policy. A Share never enters it: HTTP, SSE, Images, WS, and WS→HTTP fallback retain the Share's Provider/account/workspace, allow at most same-account refresh and same-Provider retry, and return that identity's own error instead of rotating or failing over.
