...
`staticCoverage` 只表示本地 contract 和 fixture 已覆盖；`adapterStatus: mixed` 仍会被计入 mixed/skeleton，直到真实 provider、Router Share URL 和 market URL 都有 non-stream/stream 证据。

| App 入口 | 路径 | 已有 native/static contract | 仍未真实关闭 |
| --- | --- | --- | --- |
| Claude | `/v1/messages` | Claude API/Auth/OAuth、Codex Responses、Gemini/Gemini CLI、OpenRouter、Antigravity/Agy、Ollama、Nvidia、DeepSeek API key；Kimi native Messages/count_tokens、权威目录、thinking replay 与同账号 401 已 fixture-verified；Qoder COSY 固定 site/signing、model/session/quota scope、SSE terminal 与同账号 pre-commit replay 已 fixture-verified；Kimi/GLM/MiniMax/Volcengine/MiMo API-key coding-plan 固定 route/model/quota contract 已覆盖；Kiro Claude→CodeWhisperer 与 GitHub Copilot Claude→Chat 均有 fixture-verified Native 合同；Cursor AgentService text/image/tool、精确 session/index scope、初始/恢复绝对业务输出 deadline、builtin 协议拒绝、同绑定 credential/catalog/401/429 已 fixture-verified | Kimi、Qoder 与 API-key coding plans 真实登录/订阅/inference/tool/quota 仍 pending；Cursor 仍为 Experimental/live-unverified；Bedrock 只有 SigV4/Converse request parts；DeepSeek account 仍需真实验收；Copilot github.com/GHES、Kiro 与 Cursor 真实账号验收仍 pending |
| Codex Responses | `/v1/responses` | Codex/OpenAI-compatible、OpenRouter、Ollama、Claude Messages、Gemini/Gemini CLI、Antigravity/Agy、Nvidia、DeepSeek API key；Kimi Chat bridge、权威目录、K3 effort、thinking replay 与同账号 401 已 fixture-verified；Qoder COSY Responses bridge、固定 site/signing、model/session/quota scope、SSE terminal 与同账号 pre-commit replay 已 fixture-verified；Kimi/GLM/MiniMax/Volcengine/MiMo API-key coding-plan 固定 route/model/quota contract 已覆盖；Kiro 与 GitHub Copilot 已 fixture-verified Native；Cursor Responses lifecycle、精确 continuation/tool scope、绝对 deadline、builtin 拒绝与同绑定 credential/catalog/401/429 已 fixture-verified | Kimi、Qoder 与 API-key coding plans 真实账号/订阅验收仍 pending；Cursor 仍为 Experimental/live-unverified；Bedrock planned；DeepSeek account 仍是 fallback/manual；Copilot github.com/GHES 与 Kiro 真实账号验收仍 pending |
| Codex Chat | `/v1/chat/completions` | 与 Codex Responses 同一 provider family；Kimi 精确 Coding Chat endpoint、权威目录、K3 effort、thinking replay 与同账号 401 已 fixture-verified；Qoder COSY Chat bridge、固定 site/signing、model/session/quota scope、SSE terminal 与同账号 pre-commit replay 已 fixture-verified；Kimi/GLM/MiniMax/Volcengine/MiMo API-key coding-plan 固定 route/model/quota contract 已覆盖；Kiro、GitHub Copilot 与 Cursor Chat text/tool/finish/usage 均有本地 fixture，Cursor 共用精确 scope、deadline、builtin 拒绝和同绑定 retry 合同 | Kimi、Qoder 与 API-key coding plans 真实账号/订阅验收仍 pending；Cursor 仍为 Experimental/live-unverified；Bedrock planned；DeepSeek account 仍是 fallback/manual；Copilot github.com/GHES 与 Kiro 真实账号验收仍 pending |
| Gemini | `/v1beta/*` | Gemini/Gemini CLI、Antigravity/Agy、OpenRouter、Claude Messages、Codex Responses、Ollama、Nvidia、DeepSeek API key；Kimi Chat bridge、权威目录、K3 effort、thinking replay 与同账号 401 已 fixture-verified；Qoder COSY Gemini bridge、固定 site/signing、model/session/quota scope、SSE terminal 与同账号 pre-commit replay 已 fixture-verified；Cursor Gemini emitter 与签名 Share `/v1beta/models` 精确 S2 Provider scope、跨 scope 隔离、权威空目录已 fixture-verified | Kimi 与 Qoder 真实账号验收仍 pending；Cursor 仍为 Experimental/live-unverified，需真实 stream/tool/image 证据；Bedrock planned；GitHub Copilot Gemini→Chat 与 DeepSeek account 仍是 fallback/manual；Kiro 不在 Gemini Surface 开放，保持 unsupported |
...
无真实 provider/token 时，`scripts/smoke/code-agent-regression.sh` 会运行 proxy、account domain、OAuth client、Web UI 和协议审计合同，以及可用的本地 server capability 检查；Share/market/real provider 请求会输出 skipped 或 warning，不标记真实成功。每个 Rust 过滤器会先执行 `--list` 并强制要求至少命中一条测试。
