...
Kiro text 的回归合同覆盖单一模型 resolver、绑定账号目录授权与 token limits、profile-aware 有界 discovery、request-scoped session/cache、严格工具桥与 EventStream、独立 keepalive 时钟、同账号 throttle/cooldown，以及不进入 token quota 的 supplemental credits。目录 scope 进一步覆盖 Provider revision/runtime fingerprint、Account auth/token generation、profile 与 region；成功空目录权威，仅 transient failure 可读同 scope last-known-good，坏 JSON/缺 models/超 2 MiB/认证失败/代际漂移均失败关闭，首次 401 只强刷原账号一次。2026-09-11 的 `kiro.rs@22d2c2d0695ba350890072c19990f54782827ae5` 只读差异基线又冻结 top-level auto/显式四断点、20-position lookback 与 tool group、混合 TTL/按条目续期、最新 user guardrail、上游 usage（含显式零）优先和整数比例守恒；请求热路径只写有界内存 mutation，后台 writer 用 generation-CAS SQLite transaction、合并/退避与 shutdown flush，旧 JSON 保留为回滚输入。2026-09-18 增量冻结当前请求 namespaced tool 唯一 child 恢复与歧义失败、仅 403/特定 400 的同 host profileArn 去除，以及 Kiro/Amazon Q Compact 在发网前拒绝。共享/remote cache 与 remote compaction 关闭，不采用参考的 Redis、affinity 或账号路由。KI-01..03、KI-N1/N2 为 `fixture_verified`，KI-N3 只验证关闭态；Builder ID/IdC/Social/API Key × `us-east-1`/`eu-central-1` 及 remote compaction receipt 仍分别 `live_pending`。三个 text surface 保持 `fixture_verified`/`live_pending`，Gemini 不开放。Amazon Q 已分离为独立 typed family，Kiro 凭据、目录、quota、endpoint identity 与 request origin 都不能参与 Amazon Q；两者只共享已审计的纯转换与 EventStream decoder。全程不允许账号池、轮换、跨账号目录 union 或跨 Provider failover。
...
Cursor 的 2026-09-11 回归合同在既有 OAuth CLI/API-key SDK 双 rail、ServerConfig trust、park/resume、MCP wrapper、图片与绝对 business-output deadline 上，增加 fresh exact-scope live catalog 模型语义：公开 discovery 已由 registry revision 4 标为 supported/fixture-verified；只有 API-key `/v1/models` 在同一 App、Provider revision/runtime、credential generation 与 key digest scope 中仍 fresh 时，完整 `*-fast` ID 才原样进入 protobuf，stale 或跨 scope 目录不参与推理授权。last-known-good 最多保留一小时，成功空目录权威；自包含 OmniRoute 差分 fixture 验证 ServerConfig/interaction 未知字段、重复 field 27/URL 失败关闭、Connect 分帧/partial EOF 和严格终态。293 项 Cursor 聚焦测试及双 rail loopback harness 通过，状态为 `fixture_verified`/`live_pending`。`scripts/smoke/cursor-real.mjs` 一次只接受一个 rail 的仓库外私密 digest-only receipt，两条 rail 永不互相借用 evidence 或 fallback。
...
Claude 的 2026-09-11 差异合同覆盖 native leading system hoist、main/subagent/probe/helper/count_tokens cache TTL、Claude→Chat trailing usage、structured output、顺序 interleaved tool/content blocks 与 helper request ID upstream-base 规则。usage tail 仅响应原始下游显式 opt-in，固定处于 finish 与 `[DONE]` 之间；失败前局部 usage 仍写唯一失败记录。外部 OmniRoute/CLIProxyAPI 文件只由可选 source audit 复核，不进入构建或运行时。所有行为保持固定 Provider/Account、无 pool/rotation/fallback，状态为 `fixture_verified`/`live_pending`。

Codex 的 2026-09-11 差异合同增加有深度/节点/字节预算的 Unicode property regex 清理与大型纯 const union 等价压缩，并以同一 golden 固定 HTTP/SSE/WS 的 nested error detail、sequence number、具名 tool output、首尾帧、bootstrap overload 和 WS→HTTP pre-commit 边界。GPT Image 2.5 三个 variant 分别保持 `live_pending` 且不进入 registry/UI/runtime；WS prewarm 与 2.5 quota/cache 也在真实 receipt/基准齐备前保持关闭。所有恢复仍固定原 Provider/Account 并共用总 attempt budget，post-commit 不重放。

Grok 的 2026-08-30 合同将 `/v1/models` 收敛为固定 Provider/runtime + Account + auth/token generation 的动态 entitlement 目录。成功空目录权威；只有 network/408/429/5xx 可使用同 scope、24 小时内的 stale，401/403、坏成功响应、无绑定、持久化降级或任何代际漂移均失败关闭，首次 401 仅强刷原账号一次。目录逐模型输出保守 family/capability manifest，不能由 model id 证明的媒体和搜索 entitlement 保持 unknown。异步视频 ownership 升至 schema v4，task key 覆盖 Share/用户/task kind/id/Provider/Account/auth generation/runtime/plane，v1-v3 精确迁移。

Grok 的 2026-09-11 差分合同增加 Provider 专属 opaque reasoning replay、明确拒绝恢复和 Build root-union adapter。Replay scope 覆盖 Provider/Account/auth-token generation/Share/签名用户/session-turn/model/HTTP-WS rail/upstream plane，使用 30 分钟有界 cache、CAS/tombstone，并只在成功 completed 终态提交；HTTP、CRLF 分片 SSE 与 WS loopback 覆盖并行 calls、一次同账号同 rail pre-commit 恢复及 post-commit 禁止恢复。Schema adapter 只投影可证明的 object root，混合 union、循环 ref、超限和全非 object 在网络前失败关闭。外部 `grok2api@8913b53f` 仅作可选 SHA-256 漂移审计，不引入其账号池/路由。推理、媒体和 remote compaction 的真实 receipt 分离，后者 `runtimeEnabled=false`；因此本地 GR-01..03 为 `fixture_verified`，GR-04/05 和真实 xAI 仍为 `live_pending`。
