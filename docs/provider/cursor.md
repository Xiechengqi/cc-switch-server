...
- Provider validation (`/v1/me`) and API-key model discovery (`/v1/models`) present the same audited SDK identity as the API-key AgentService rail: Bearer authentication plus `x-cursor-client-type=sdk`, the shared SDK client version, and `x-ghost-mode=true`. A 401 means the key was rejected; a 403 is reported separately as API-access or SDK-client permission denial instead of being mislabeled as an invalid key.
