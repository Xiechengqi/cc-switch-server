...
| 请求生命周期内存预算 | Codex HTTP 请求和每个 Responses WebSocket turn 使用独立的 resident-memory budget，统一核算原始/解压/规范化 body、transport pending、语义 prelude、工具参数、规范化事件和 WS 队列。由 `requestBodyLimits.memoryBudgetMb` 或 `CC_SWITCH_REQUEST_MEMORY_BUDGET_MB` 配置，范围 16-1024 MiB、默认 256 MiB；耗尽返回稳定的 HTTP 503（`Retry-After: 1`、`cc_switch_request_memory_exhausted`）或 WS error + Close，取消当前上游且不触发 replay/换账号。指标只含组件、结果和字节数，不含请求内容或身份；改动需重启进程 |
