...
| 请求生命周期内存预算 | Codex HTTP 请求、每个 Codex Responses WebSocket turn，以及精确绑定到 `antigravity_oauth` / `agy_oauth` / `claude_oauth` 的 HTTP 请求使用独立的 resident-memory budget；普通 `claude`、`claude_auth` 与 `gemini_cli` 不会因 App 相同而启用。通用核算原始/解压/规范化 body、transport pending、语义 prelude 与工具参数；Codex 另核算规范化事件和 WS 队列，Antigravity 另核算 reasoning replay 的 cache snapshot/流 accumulator 以及 grounding/citation/web-search bridge 状态，Claude OAuth 另核算压缩 SSE wire 与 decoded body、Anthropic/terminal/usage decoder pending state、tool partial JSON、工具名补丁和 stream transform retained state。由 `requestBodyLimits.memoryBudgetMb` 或 `CC_SWITCH_REQUEST_MEMORY_BUDGET_MB` 配置，范围 16-1024 MiB、默认 256 MiB；耗尽返回稳定的 HTTP 503（`Retry-After: 1`、`cc_switch_request_memory_exhausted`）或已提交 SSE 的 503 error terminal（Codex WS 为 error + Close），取消当前上游且不触发 replay/换账号。指标只含组件、结果和字节数，不含请求内容或身份；改动需重启进程 |
