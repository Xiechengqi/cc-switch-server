...
import { getWebRuntimeContext, isServerWebRuntime } from "@/lib/runtime";
    ...
    const load = async () => {
      try {
        if (isServerWebRuntime()) {
          ...
          const configDir = context.runtime?.configDir ?? "";
          if (!active) return;
          defaultsRef.current = {
            appConfig: configDir,
            claude: "",
          ...
          initialAppConfigDirRef.current = configDir;
          setResolvedDirs(defaultsRef.current);
          return;
        }
