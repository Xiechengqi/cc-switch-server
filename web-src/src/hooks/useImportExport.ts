...
export interface UseImportExportResult {
  ...
  resetStatus: () => void;
  serverFileInputRef?: React.RefObject<HTMLInputElement | null>;
  onServerFileSelected?: (file: File | null) => void;
}
...
export function useImportExport(
  ...
  const handleServerFileSelected = useCallback(
    async (file: File | null) => {
      ...
      try {
        ...
        setSelectedFileContent(btoa(binary));
        setSelectedFile(file.name);
        setStatus("idle");
        setErrorMessage(null);
      } catch (error) {
        console.error("[useImportExport] Failed to read selected file", error);
        toast.error(
          t("settings.selectFileFailed", {
            defaultValue: "选择文件失败",
          }),
  ...
  const importConfig = useCallback(async () => {
    if (!selectedFile || (serverMode && !selectedFileContent)) {
      toast.error(
        t("settings.selectFileFailed", {
    ...
    try {
      const result = serverMode
        ? await invokeCommand<{
            ...
            message?: string;
            backupId?: string;
          }>("import_config_from_file", { contentBase64: selectedFileContent })
  ...
  }, [isImporting, onImportSuccess, selectedFile, selectedFileContent, serverMode, t]);
  ...
  const exportConfig = useCallback(async () => {
    try {
      if (serverMode) {
        ...
        anchor.href = url;
        anchor.download = result.fileName ?? "cc-switch-server-export.json";
        anchor.click();
  ...
  return {
    ...
    exportConfig,
    resetStatus,
    serverFileInputRef: fileInputRef,
