...
export const failoverApi = {
  ...
  async resetCircuitBreaker(
    ...
    return invokeCommand("reset_circuit_breaker", { providerId, appType });
  },

  // 获取熔断器配置
  async getCircuitBreakerConfig(): Promise<CircuitBreakerConfig> {
    return invokeCommand("get_circuit_breaker_config");
  },
  ...
  async updateCircuitBreakerConfig(
    ...
    return invokeCommand("update_circuit_breaker_config", { config });
  },
  ...
  async getCircuitBreakerStats(
    ...
    return invokeCommand("get_circuit_breaker_stats", { providerId, appType });
  },
  ...
  async getFailoverQueue(appType: string): Promise<FailoverQueueItem[]> {
    return invokeCommand("get_failover_queue", { appType });
  },

  // 获取可添加到队列的供应商（不在队列中的）
  async getAvailableProvidersForFailover(appType: string): Promise<Provider[]> {
    return invokeCommand("get_available_providers_for_failover", { appType });
  },

  // 添加供应商到故障转移队列
  async addToFailoverQueue(appType: string, providerId: string): Promise<void> {
    return invokeCommand("add_to_failover_queue", { appType, providerId });
  },
  ...
  async removeFromFailoverQueue(
    ...
    return invokeCommand("remove_from_failover_queue", { appType, providerId });
  },

  // 获取指定应用的自动故障转移开关状态
  async getAutoFailoverEnabled(appType: string): Promise<boolean> {
    return invokeCommand("get_auto_failover_enabled", { appType });
  },
  ...
  async setAutoFailoverEnabled(
    ...
    return invokeCommand("set_auto_failover_enabled", { appType, enabled });
  },
};
